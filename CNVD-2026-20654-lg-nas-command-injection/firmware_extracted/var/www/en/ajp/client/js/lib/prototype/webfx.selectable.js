...
SelectableElements = Class.create({
	...
	initSelectableItems: function(oElement, bMultiple, dragSelectionElement) {
		...
		//by soo. drag selection disabled  
	...
	dragStart : function(e){
		...
		if(this.dragSelectionElement.scrollHeight > h){
			...
			//alert("There is a scroll bar :" + this.dragSelectionElement.scrollHeight);
			if(this.originalX > (this.dSEPosition[0]+this.dSEDimension.width) - 18) return;
		}
		
		// KHJ20091031 prevent text selection on IE, safari, chrome
		...
		if(!this.divSelector){
			// KHJ20091031 prevent non selected item text-selection
			if ( Prototype.Browser.IE )
			{
				this.divSelector = new Element('div',
					{style:"border : 1px dotted #999; background-color:#ddd; filter:alpha(opacity=50);opacity: 0.5;-moz-opacity:0.5;z-index:100000;position:fixed;top:0px;left:0px;height:0px;width:0px;"}
					);
			}
			else
			{
	...
	drag : function(e){
		...
		// KHJ20091031 Hong & KHJ add scroll offset for drag and drom with scrolling
		var scrollOffset = $(this.dragSelectionElement).scrollTop;		
		var allItems = this.getItems();
		var minX = left;
		var maxX = left+width;
		var minY = top + scrollOffset;
		var maxY = top + height + scrollOffset;
		...
		for(var i=0; i<allItems.length;i++){
			...
			&& ( (y1>=minY && y1<=maxY) || (y2>=minY && y2<=maxY) || (minY>=y1 && maxY<=y2) ))
			{
				...
				// KHJ20091031 debug control key press detect code
	...
	dragEnd : function(e){
		...
		// KHJ20091031 allow text selection on IE, safari, chrome
		if ( Prototype.Browser.IE || Prototype.Browser.WebKit )
		{
			document.onselectstart = function () { return false; }; //null;
		}
	...
	click: function (e) {
		//by soo. 썸네일 view에서 다른영역 선택시 마우스 unclick해줌
		/*if (Prototype.Browser.IE)
		{
			if((ajaxplorer.filesList.getDisplayMode() == "thumb")&&((e.srcElement.tagName == "DIV") || (e.srcElement.tagName == "div")))
			{
				var allItems = this.getItems();
				for(var i=0; i<allItems.length;i++){
					this.setItemSelected(allItems[i], false);
					//$(allItems[i]).removeClassName("selected");
					//$(allItems[i]).removeClassName("selected-focus");
				}
		...
		{
			if((ajaxplorer.filesList.getDisplayMode() == "thumb")&&((e.target.tagName == "DIV") || (e.target.tagName == "div")))
			{
				var allItems = this.getItems();
				for(var i=0; i<allItems.length;i++){
					this.setItemSelected(allItems[i], false);
