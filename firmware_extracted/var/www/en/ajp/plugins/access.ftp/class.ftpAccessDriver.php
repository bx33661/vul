...
class ftpAccessDriver extends  AbstractAccessDriver
{
	...
	*/
		...
		{			
			...
			case "ls":
				...
				foreach ($reps as $repIndex => $repName)
				{
					...
					if(is_string($repName) && (preg_match("/\.zip$/",strtolower($repName)) == 1 && $skipZip)) continue;
					$attributes = "";
					if($searchMode)
					{
					...
					else 
					{
						...
						if(!$completeMode){
							...
							if(preg_match("/\.zip$/",strtolower($repName)) == 1){
	...
	function uploadActions($action, $httpVars, $filesVars){
				...
				@ftp_put($this->connect,$this->secureFtpPath($this->path.base64_decode(str_replace(" ", "+", $fData['destination']))."/".$fData['name']),$fData['tmp_name'], FTP_BINARY);
				...
				exit(1);
                        ...
                        case "upload":
                                ...
                                if(isSet($fileVars["Filedata"])){
                                        $fancyLoader = true;
                                        if($httpVars['dir']!="") $httpVars['dir'] = "/".base64_decode(str_replace(" ", "+", $httpVars['dir']));
                                }
		...
		foreach($contents as $entry)
			...
			if($file!="." && $file!=".." )
			{
				...
				else
				{
					...
					else if(preg_match("/\.zip$/",strtolower($file)) == 1 && ConfService::zipEnabled()){
	...
	function createEmptyFile($crtDir, $newFileName)
	{
		...
		if($fp)
		{
			if(preg_match("/\.(html|htm)$/",strtolower($newFileName)) == 1)
			{
    ...
    function sendRemoteFile($file, $forceDownload)
    {
		...
		header("Content-type:text/plain");			
		if(preg_match("/\.(jpg|jpeg|png|bmp|mng|gif)$/", strtolower($file)) !== FALSE)
		{
