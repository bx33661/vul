...
function WebFXLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon, queryParameters) {
	...
	if (this.open)
		_startLoadXmlTree(this.src, this);	//by soo. loading시 'ls'2번중 한번 제거
		//;
	else {
...
function _startLoadXmlTree(sSrc, jsNode) {
	if (jsNode.loading || jsNode.loaded)
		return;
	jsNode.loading = true;
	//var encoded_uri = encodeURI(decodeURIComponent(sSrc)).replace(/&amp;/g, '%26');
	//var connexion = new Connexion(encoded_uri);
...
function _xmlTreeToJsTree(oNode, parentNode) {
	...
	if ( ajaxplorer.actionBar.treeCopyActive == false || ajaxplorer.actionBar.treeCopyActive == undefined )
	{
		webFXTreeHandler.ajxpNodes[folderFullName] = jsNode.id;
	}
