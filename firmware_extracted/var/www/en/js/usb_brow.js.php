...
//=======================================================//
// Load connected USB device information
//=======================================================//
var usb = {
	connected_dev : [],
	cConnected : 0,
	...
	still_exist : 0,
	area_id : "id_usb_dev",
	select_id : "id_sel_dev",
	get_dev_list : function(){
		var _php = "../php/usb_get_dev_list.php";
		sendRequest(on_get_dev, "&dev_type=usb_new", "post", _php, true, true);
		//sendRequest(on_get_dev, "&dev_type=usb", "post", _php, true, true);
		
		function on_get_dev(oj){
			var res=decodeURIComponent(oj.responseText);
			eval('var _res = '+res);
			//usb.cConnected = _res.count;
			usb.cConnected = _res.length;
			
			if(usb.cConnected == 0){
				usb.make_select_box('no ext');
				return;
			}else{
				// Sorting device list
				var _tmp = [];
				for(i=0;_res[i];i++){
					_res[_res[i].device] = _res[i];
					_tmp[i] = _res[i].device;
				}
				...
				usb.connected_dev = [];
				for(i=0;_tmp[i];i++){
					usb.connected_dev[i] = _res[_tmp[i]];
				}
			...
			if(res.search(/no usb device/i)>-1){
			...
			var _arr = res.split("\n");
			usb.connected_dev = [];
			for(var i=0; _arr[i]; i++){
				var _tmp = _arr[i].split(" ");
				_tmp[1] = _tmp[1].split("|");
				_tmp[1] = _tmp[1][0];
				_tmp = {node : _tmp[0], device : _tmp[1]};
				usb.connected_dev[i] = _tmp;
				debug("device "+i+":"+usb.connected_dev[i].node+" "+usb.connected_dev[i].device);
			}
			usb.cConnected = i+1;
			usb.make_select_box();
		}
	...
	make_select_box : function(mode){
		if(mode=='no ext'){
			$(this.area_id).innerHTML = '';
			$('idPathEsata').innerHTML = "<?php echo lang_get('usb_msg_4')?>";
			$('file_box_esata').innerHTML = "";
			$('directory_info_esata').innerHTML = "";
			return;
		}
		
		var _tmp = "<select id='"+this.select_id+"' onchange='usb.set_dev();'>";
		this.still_exist=0; // Flag for selected device is still connected
			 for(var i=0;this.connected_dev[i];i++){
				if(this.connected_dev[i].device == this.selected_dev.device) this.still_exist = 1;
			 	
			 }
				
				if(this.still_exist == 1){
					// After First Loading
					for(var i=0;this.connected_dev[i];i++){	
					var _oj = this.connected_dev[i];
							
							// Check connected_dev
							if(this.selected_dev.device == _oj.device){
								_tmp += "<option value='"+_oj.node+"' selected >"+_oj.device+"</option>";
								this.selected_dev = _oj;
								
							}else{
								_tmp += "<option value='"+_oj.node+"' >"+_oj.device+"</option>";
							}
		 			}
		 			refresh_file_box_esata();
		 		}
		      else if(this.still_exist == 0){
		      	for(var i=0;this.connected_dev[i];i++){
						var _oj = this.connected_dev[i];
		...
		_tmp += "</select>";
		//_tmp += "<input type='button' value='" + "<?php echo lang_get('usb_3')?>" +"' onclick='usb.connect();' />";
		$(this.area_id).innerHTML = _tmp;
	},
	set_dev : function(){
		var _oj = document.getElementById(this.select_id);
		this.selected_dev = this.connected_dev[_oj.selectedIndex];
		this.connect();
		debug("selected device node : "+this.selected_dev.node);
	},
	connect : function(){
		sendRequest(on_usb_connect,"&mode=usb_connect&device="+this.selected_dev.device,"post","../php/esata_get_info.php",true,true);
		
		function on_usb_connect(oj){
			var res=decodeURIComponent(oj.responseText);
			debug("return:"+res);
			
			if(res.search(/no external device/i)>-1){
				alert("<?php echo lang_get('usb_msg_4')?>");
				return false;
			}else{
				esata.is_connected = true;
				$('id_td_dev').innerHTML = usb.selected_dev.device;
				refresh_file_box_esata();
			}
		}
	},
	disconnect : function(){
		...
		$('idPathEsata').innerHTML = "<?php echo lang_get('usb_msg_13')?>";
		$('file_box_esata').innerHTML = '';
		$('directory_info_esata').innerHTML = '';
		$('id_td_dev').innerHTML = 'Disconnected';
		alert("<?php echo lang_get('usb_msg_12')?>");
		
		/*
		var _arr = this.connected_dev;
		var _length = _arr.length;
		var _sel = $(this.select_id).selectedIndex;
		this.selected_dev = [];
		this.connected_dev = [];
		var _cnt = 0;
		for(var i=0;i<_length;i++){
			...
			this.connected_dev[_cnt] = _arr[i];
			_cnt++;
		}
		if(this.connected_dev.length==0){
			this.make_select_box('no ext');
		}else{
			this.make_select_box();
		}*/
		this.connected_dev = [];
	  this.cConnected = 0;
	  this.selected_dev = [];
...
var esata = {
	connect_type : "",
	"is_connected" : false ,
	"stat_line" : ["Searching e-SATA device connected...","No e-SATA device is connected.",
		"e-SATA device is connected."] ,
	"connect" : function(mode){
		if(mode=="conn" || !this.is_connected){
			this.connect_type = "e_sata";
			var _mode = "mount_point";
			var _msg = "Searching e-SATA device connected...";
		}else if(mode=="refr"){
			this.connect_type = "";
			var _mode = "mounted";
			var _msg = "Checking e-SATA device connected...";
		}
		var _cmd = "&mode="+_mode;
		...
		function on_connect(oj){
			...
			debug(res);
			if( res == "No e-SATA" ){
				document.getElementById("idPathEsata").innerHTML = "No e-SATA device is connected.";
				$('file_box_esata').innerHTML = "";
				$('directory_info_esata').innerHTML = "";
				...
				document.getElementById("idPathEsata").innerHTML = "e-SATA device is connected.";
				esata["is_connected"] = true;
				is_loading_esata = false;
				//startLoad_esata();
		...
		function on_esata_cancel(oj){
			...
			switch(_res.result){
				case '1':
					...
					//Juny : wait refresh function until cmscopy's cancel process is finished
					//refresh_file_box();
					//refresh_file_box_esata();
					return false;
				break;
	...
	disconnect : function(){
		$('idPathEsata').innerHTML = 'No e-SATA';
		$('file_box_esata').innerHTML = '';
		$('directory_info_esata').innerHTML = '';
	}
}
...
var nasFreeCap = {
	...
	get : function(){
		...
		function on_nasFreeCap_get(oj){
			var res=decodeURIComponent(oj.responseText);
			nasFreeCap.capacity = parseFloat(res,10);
...
var esataFreeCap = {
	capacity : 0,
	get : function(dev_node){
		if(dev_node){
			var _cmd = "&mode=ext&node="+dev_node;
		}else{
			var _cmd = "&mode=esata";
		}
		var _php = "../php/esata_chk_free_cap.php";
		...
		function on_esataFreeCap_get(oj){
			var res=decodeURIComponent(oj.responseText);
			esataFreeCap.capacity = parseFloat(res,10);
...
var folder_size_ext = {
	...
	loading : function(id){
		...
		sendRequest(on_loading_ext, '&device='+usb.selected_dev.device+'&folders='+_oj.path+_oj.name, 'post', this.loading_php, true, true);
		
		document.getElementById('idBurnSize').innerHTML = "<?php echo lang_get('usb_msg_16')?> "+usb.selected_dev.device+ " <?php echo lang_get('usb_msg_16_1')?>" + " : " + "<?php echo lang_get('common_loading')?>";
		_oj.selected = 'yes';
	...
	loading_all : function(){
		this.status = 'Loading';
		var _folders = this.make_folders_str();
		sendRequest(on_loading, '&device='+usb.selected_dev.device+'&folders='+_folders, 'post', this.loading_php, true, true);
	...
	show_selected_total_size : function(){
		...
		document.getElementById('idBurnSize').innerHTML = "<?php echo lang_get('usb_msg_16')?> "+usb.selected_dev.device+ " <?php echo lang_get('usb_msg_16_1')?>" + " : " +bytesHumanReadable(_total_size);
	},
	show : function(id){
		if(!document.getElementById(id).checked){
			gAllFileList_esata[id.match(/\d+/)].selected = 'no';
...
function show_help()
{
	var _win = window.open('../help/mobile/help_usb_brow.html','Help','titlebar=no,scrollbars=yes,status=no,toolbar=no,resizable=yes,location=no,menu=no,width=600px,height=500px,left=540,top=240');
    _win.focus();
	hPopWin = _win;
}
