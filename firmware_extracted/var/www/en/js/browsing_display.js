...
function refresh_file_box(){
	var strResponseURL = remote_php_path+'?action=show_me_files&mode='+display_mode+'&sort_cond='+sort_cond+'&file_or_dir_only='+file_or_dir_only;
	...
	var httpObj = new Ajax.Request   (
			...
			onSuccess:function (responseHttpObj) {
				...
				}else if(display_mode == 'test'){
					display_files_list_mode_test(responseHttpObj.responseText);
				}else{
					display_files_icon_mode(responseHttpObj.responseText);
				}
			},
...
//파일 및 디렉토리를 아이콘으로 표시
function display_files_icon_mode(response){
	var info = eval('(' + response + ')');
	var total_size = info.total_size;
	var i=0;
	var data = null;
	var total_display_html = '';
	var display_html = '';
	var link = '';
	var checkbox_html = '';
	var rename_html = '';
	...
	var img_display = '';
	var obj_cnt = 0;
	var basic_html = '<div class="file_obj"><center>'
						+'<div class="file_obj_icon"><table class="simple_table"><tr><td class="file_obj_icon">#icon#</td></tr></table></div>'
						+'<div class="file_obj_text">#file_name#</div>'
						+'<div class="file_obj_action">#action#</div>'
					+'</center></div>';
	
	//Directories
	for(i=0;i<info.dirs.length;i++){
		data = info.dirs[i];
		display_html = basic_html;
		link = '<a href="" onclick="move_dir(\''+data.encoded_file_name+'\');return false;">'+data.file_name.escapeHTML()+'</a>';
		rename_html = getRenameButton('directory',data.file_name);
		delete_html = getDeleteButton('directory',data.file_name);
		if(data.selected.length>0){
			display_html=display_html.replace('<div class="file_obj_icon">','<div class="file_obj_icon_selected">');
			checkbox_html = '<input type="checkbox" name="directory[]" id="chk_'+obj_cnt+'" value="'+data.file_name+'" checked>';
		}else{
			checkbox_html = '<input type="checkbox" name="directory[]" id="chk_'+obj_cnt+'" value="'+data.file_name+'">';
		}
		
		display_html=display_html.replace('#file_name#',checkbox_html+link);
		display_html=display_html.replace('#icon#','directory');
		display_html=display_html.replace('#action#',rename_html+'&nbsp;&nbsp;'+delete_html);
		
		total_display_html += display_html;
		obj_cnt++;
	}
	...
	for(i=0;i<info.files.length;i++){
		...
		delete_html = getDeleteButton('file',data.file_name);
		...
		if(data.type == 'image'){
			img_display = '<img src="'+data.url+'" onload="resizeImg(this)">';
			display_html=display_html.replace('#icon#',img_display);
		}
		total_display_html += display_html;
		obj_cnt++;
	}
	$('file_box').innerHTML = '<form name="files_slc_fm" id="files_slc_fm" method="POST" onsubmit="return false;">'+total_display_html+'</form>';
...
function display_files_list_mode(response){
	...
	for(i=0;i<info.dirs.length;i++){
		...
		body_row = body_row.replace('#type#','directory');
	...
	for(i=0;i<info.files.length;i++){
		...
		body_row = body_row.replace('#type#',data.subtype+'&nbsp;'+data.type);
		body_row = body_row.replace('#time#',data.date+' '+data.time);
